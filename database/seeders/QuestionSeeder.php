...
class QuestionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->guruId    = User::where('email', 'guru1@cabt.local')->value('id') ?? $this->createdBy;
        ...
        // ─── 3. Tipe Soal BS & CLOZE ──────────────────────────────────────
        $this->seedBSCloze($categories);

        // ─── 4. Demo Visibilitas & Audio ──────────────────────────────────
        $this->seedVisibilitasAudioDemo($categories);
    ...
    private function seedIPA(Category $cat, array $cats): void
    {
        ...
        $this->makeIsian(
            ...
            ['fotosintesis', 'fotosintesis (photosynthesis)'],
            'mudah',
            1
        );
    ...
    private function seedTIK(Category $cat, array $cats): void
    {
        ...
        $this->makeIsian(
            ...
            ['bug', 'kesalahan', 'error', 'bug (kesalahan)'],
            'mudah',
            1
        );
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // BS & CLOZE (tambahan tipe soal Fase 11)
    // ─────────────────────────────────────────────────────────────────────────

    private function seedBSCloze(array $cats): void
    {
        $alj = $cats['mtkAljabar']->id;
        $geo = $cats['mtkGeom']->id;
        $sta = $cats['mtkStat']->id;
        ...
        // ── Benar/Salah — Matematika ──────────────────────────────────────
        $this->makeBS(
            $alj,
            '<p>Bilangan prima adalah bilangan yang hanya habis dibagi oleh 1 dan dirinya sendiri.</p>',
            true,
            'mudah',
            1,
            '<p>Pernyataan BENAR. Contoh: 2, 3, 5, 7, 11, 13, ...</p>'
        );
        $this->makeBS(
            $geo,
            '<p>Luas persegi panjang dengan panjang 8 cm dan lebar 5 cm adalah 80 cm².</p>',
            false,
            'mudah',
            1,
            '<p>Pernyataan SALAH. Luas = 8 × 5 = 40 cm², bukan 80 cm².</p>'
        );
        $this->makeBS(
            $sta,
            '<p>Median dari data 3, 5, 7, 9, 11 adalah 7.</p>',
            true,
            'mudah',
            1,
            '<p>Pernyataan BENAR. Data sudah urut; elemen tengah (ke-3 dari 5) = 7.</p>'
        );
        $this->makeBS(
            $alj,
            '<p>Nilai dari 2³ × 2² = 2⁶.</p>',
            false,
            'sedang',
            1,
            '<p>Pernyataan SALAH. Perkalian eksponen dengan basis sama: 2³ × 2² = 2^(3+2) = 2⁵ = 32, bukan 2⁶.</p>'
        );

        // ── Benar/Salah — IPA ────────────────────────────────────────────
        $this->makeBS(
            $fis,
            '<p>Satuan gaya dalam Sistem Internasional (SI) adalah Newton (N).</p>',
            true,
            'mudah',
            1
        );
        $this->makeBS(
            $bio,
            '<p>Fotosintesis terjadi di mitokondria sel tumbuhan.</p>',
            false,
            'mudah',
            1,
            '<p>Pernyataan SALAH. Fotosintesis terjadi di <strong>kloroplas</strong>, bukan mitokondria.</p>'
        );
        $this->makeBS(
            $kim,
            '<p>Rumus kimia air adalah H₂O.</p>',
            true,
            'mudah',
            1
        );
        $this->makeBS(
            $fis,
            '<p>Pada gerak lurus beraturan (GLB), percepatan benda adalah nol.</p>',
            true,
            'mudah',
            1,
            '<p>Pernyataan BENAR. Pada GLB, kecepatan konstan sehingga percepatan = 0.</p>'
        );

        // ── Benar/Salah — Bahasa Indonesia ───────────────────────────────
        $this->makeBS(
            $bind,
            '<p>Kalimat "Ibu memasak nasi goreng di dapur" adalah contoh kalimat aktif.</p>',
            true,
            'mudah',
            1,
            '<p>Pernyataan BENAR. Subjek (Ibu) melakukan tindakan (memasak).</p>'
        );
        $this->makeBS(
            $bind,
            '<p>Majas personifikasi adalah majas yang membandingkan dua benda berbeda menggunakan kata "seperti" atau "bagaikan".</p>',
            false,
            'sedang',
            1,
            '<p>Pernyataan SALAH. Yang dimaksud adalah majas <strong>simile</strong>. Personifikasi memberikan sifat manusia kepada benda mati.</p>'
        );

        // ── Benar/Salah — Sejarah ─────────────────────────────────────────
        $this->makeBS(
            $sej,
            '<p>Proklamasi Kemerdekaan Indonesia dibacakan oleh Ir. Soekarno dan Drs. Mohammad Hatta pada tanggal 17 Agustus 1945.</p>',
            true,
            'mudah',
            1
        );
        $this->makeBS(
            $sej,
            '<p>Sumpah Pemuda dikumandangkan pada tanggal 28 Oktober 1945.</p>',
            false,
            'mudah',
            1,
            '<p>Pernyataan SALAH. Sumpah Pemuda diikrarkan pada <strong>28 Oktober 1928</strong>, bukan 1945.</p>'
        );

        // ── Benar/Salah — TIK ────────────────────────────────────────────
        $this->makeBS(
            $jar,
            '<p>Alamat IP kelas C menggunakan rentang 192.0.0.0 hingga 223.255.255.255.</p>',
            true,
            'sedang',
            1
        );
        $this->makeBS(
            $alg,
            '<p>Dalam pemrograman, tipe data "integer" digunakan untuk menyimpan bilangan desimal (berkoma).</p>',
            false,
            'mudah',
            1,
            '<p>Pernyataan SALAH. Integer untuk bilangan <strong>bulat</strong>. Bilangan desimal menggunakan <strong>float</strong> atau <strong>double</strong>.</p>'
        );

        // ── CLOZE — Matematika ────────────────────────────────────────────
        $this->makeCloze(
            $alj,
            '<p>Diketahui barisan aritmetika dengan suku pertama [1] dan beda [2]. Rumus suku ke-n adalah Uₙ = a + (n−1)b. Suku ke-5 dari barisan ini adalah [3].</p>',
            [
                ['urutan' => 1, 'placeholder' => 'a', 'jawaban_benar' => '3', 'keywords_json' => null, 'case_sensitive' => false],
                ['urutan' => 2, 'placeholder' => 'b', 'jawaban_benar' => '4', 'keywords_json' => null, 'case_sensitive' => false],
                ['urutan' => 3, 'placeholder' => 'U5', 'jawaban_benar' => '19', 'keywords_json' => null, 'case_sensitive' => false],
            ],
            'sedang',
            3,
            '<p>Uₙ = 3 + (n−1)×4. U₅ = 3 + 4×4 = 3 + 16 = 19.</p>'
        );
        $this->makeCloze(
            $sta,
            '<p>Rata-rata (mean) dari sekumpulan data diperoleh dengan cara membagi [1] semua data dengan [2] data tersebut.</p>',
            [
                ['urutan' => 1, 'placeholder' => 'operasi', 'jawaban_benar' => 'jumlah', 'keywords_json' => '["total","penjumlahan"]', 'case_sensitive' => false],
                ['urutan' => 2, 'placeholder' => 'n', 'jawaban_benar' => 'banyaknya', 'keywords_json' => '["jumlah","cacah"]', 'case_sensitive' => false],
            ],
            'mudah',
            2
        );

        // ── CLOZE — Bahasa Indonesia ──────────────────────────────────────
        $this->makeCloze(
            $bind,
            '<p>Teks yang bertujuan meyakinkan pembaca terhadap suatu pendapat atau gagasan menggunakan fakta dan argumen disebut teks [1]. Salah satu cirinya adalah menggunakan [2] yang bersifat faktual untuk memperkuat argumen.</p>',
            [
                ['urutan' => 1, 'placeholder' => 'jenis teks', 'jawaban_benar' => 'eksposisi', 'keywords_json' => '["argumentasi","persuasi"]', 'case_sensitive' => false],
                ['urutan' => 2, 'placeholder' => 'unsur', 'jawaban_benar' => 'data', 'keywords_json' => '["bukti","fakta","evidensi"]', 'case_sensitive' => false],
            ],
            'sedang',
            2,
            '<p>Teks eksposisi menggunakan data/fakta sebagai bukti pendukung argumentasi.</p>'
        );
        $this->makeCloze(
            $bind,
            '<p>Penulisan huruf kapital digunakan pada awal [1] dan pada setiap kata nama [2] termasuk nama orang, kota, dan instansi.</p>',
            [
                ['urutan' => 1, 'placeholder' => 'posisi', 'jawaban_benar' => 'kalimat', 'keywords_json' => '["paragraf","alinea"]', 'case_sensitive' => false],
                ['urutan' => 2, 'placeholder' => 'jenis kata', 'jawaban_benar' => 'diri', 'keywords_json' => '["orang","tokoh","nama diri","proper noun"]', 'case_sensitive' => false],
            ],
            'mudah',
            1
        );
        ...
        $this->makeCloze(
            $fis,
            '<p>Hukum Newton II menyatakan bahwa resultan gaya (F) sama dengan massa (m) dikalikan dengan [1]. Jika massa benda 5 kg dan percepatan 3 m/s², maka gaya yang bekerja adalah [2] Newton.</p>',
            [
                ['urutan' => 1, 'placeholder' => 'besaran fisika', 'jawaban_benar' => 'percepatan', 'keywords_json' => '["akselerasi","a"]', 'case_sensitive' => false],
                ['urutan' => 2, 'placeholder' => 'angka', 'jawaban_benar' => '15', 'keywords_json' => null, 'case_sensitive' => false],
            ],
            'mudah',
            2,
            '<p>F = m × a = 5 × 3 = 15 N</p>'
        ...
        $this->makeCloze(
            $bio,
            '<p>Fotosintesis terjadi di organel yang disebut [1]. Reaksi ini mengubah energi [2] menjadi energi kimia yang tersimpan dalam bentuk glukosa.</p>',
            [
                ['urutan' => 1, 'placeholder' => 'organel', 'jawaban_benar' => 'kloroplas', 'keywords_json' => null, 'case_sensitive' => false],
                ['urutan' => 2, 'placeholder' => 'sumber energi', 'jawaban_benar' => 'cahaya', 'keywords_json' => '["matahari","sinar","light","sinar matahari"]', 'case_sensitive' => false],
            ],
            'mudah',
            2
        );
        ...
        $this->makeCloze(
            $sej,
            '<p>Proklamasi Kemerdekaan Indonesia dibacakan oleh Ir. [1] dan Drs. Mohammad Hatta di Jalan Pegangsaan Timur No. 56, Jakarta, pada tanggal [2] Agustus 1945.</p>',
            [
                ['urutan' => 1, 'placeholder' => 'nama proklamator', 'jawaban_benar' => 'Soekarno', 'keywords_json' => '["Sukarno","Ir. Soekarno"]', 'case_sensitive' => false],
                ['urutan' => 2, 'placeholder' => 'tanggal', 'jawaban_benar' => '17', 'keywords_json' => null, 'case_sensitive' => false],
            ],
            'mudah',
            2
        );
        ...
        $this->makeCloze(
            $jar,
            '<p>Model [1] adalah model referensi jaringan yang terdiri dari 7 lapisan. Lapisan teratas (Application Layer) bertanggung jawab untuk interaksi langsung dengan aplikasi pengguna, sedangkan lapisan terbawah disebut lapisan [2].</p>',
            [
                ['urutan' => 1, 'placeholder' => 'nama model', 'jawaban_benar' => 'OSI', 'keywords_json' => null, 'case_sensitive' => true],
                ['urutan' => 2, 'placeholder' => 'nama lapisan', 'jawaban_benar' => 'Physical', 'keywords_json' => '["Fisik","physical layer","lapisan fisik"]', 'case_sensitive' => false],
            ],
            'sedang',
            2
        );
        $this->makeCloze(
            $alg,
            '<p>Dalam pemrograman, struktur pengulangan yang memeriksa kondisi di awal sebelum menjalankan blok kode disebut [1]. Sedangkan struktur yang menjalankan blok kode minimal satu kali sebelum memeriksa kondisi disebut [2].</p>',
            [
                ['urutan' => 1, 'placeholder' => 'tipe loop', 'jawaban_benar' => 'while', 'keywords_json' => '["while loop","for loop","for","pre-test loop"]', 'case_sensitive' => false],
                ['urutan' => 2, 'placeholder' => 'tipe loop', 'jawaban_benar' => 'do-while', 'keywords_json' => '["do while","repeat until","post-test loop"]', 'case_sensitive' => false],
            ],
            'sedang',
            2
        );

        $this->command->line('  <info>✓</info> Soal BS dan CLOZE berhasil dibuat.');
    }

    // ─────────────────────────────────────────────────────────────────────────
    // DEMO VISIBILITAS & AUDIO (Fase 18)
    // ─────────────────────────────────────────────────────────────────────────

    private function seedVisibilitasAudioDemo(array $cats): void
    {
        $bind = $cats['bind']->id;
        $mtk  = $cats['mtkAljabar']->id;
        $tik  = $cats['tikAlg']->id;
        $ipa  = $cats['ipaFis']->id;

        // ── Soal INTERNAL (dibuat oleh guru, visible ke semua guru) ────────
        $internals = [
            [
                'kategori_id'       => $bind,
                'tipe'              => Question::TIPE_PG,
                'teks_soal'         => '<p>[INTERNAL] Kata yang berasal dari bahasa Arab dan sudah diserap ke dalam bahasa Indonesia disebut kata ...</p>',
                'tingkat_kesulitan' => 'sedang',
                'bobot'             => 1,
                'visibilitas'       => Question::VISIBILITAS_INTERNAL,
            ...
            [
                'kategori_id'       => $mtk,
                'tipe'              => Question::TIPE_ISIAN,
                'teks_soal'         => '<p>[INTERNAL] Hasil dari 7² − 3² adalah ...</p>',
        ...
        foreach ($internals as $data) {
            if (Question::where('teks_soal', $data['teks_soal'])->exists()) {
                continue;
            }
            $q = Question::create([
            ...
            foreach ($data['options'] ?? [] as $i => [$kode, $teks, $benar]) {
                QuestionOption::create([
                    'question_id'  => $q->id,
                    'kode_opsi' => $kode,
                    'teks_opsi'    => $teks,
        ...
        // ── Soal PUBLIK (visible ke seluruh sekolah) ──────────────────────
        $publiks = [
            [
                'kategori_id'       => $tik,
                'tipe'              => Question::TIPE_PG,
                'teks_soal'         => '<p>[PUBLIK] Di antara berikut ini, manakah yang merupakan bahasa pemrograman tingkat tinggi?</p>',
                'tingkat_kesulitan' => 'mudah',
                'bobot'             => 1,
                'visibilitas'       => Question::VISIBILITAS_PUBLIK,
                'created_by'        => $this->guruId,
                'options'           => [
                    ['A', 'Assembly', false],
                    ['B', 'Machine Code', false],
                    ['C', 'Python', true],
                    ['D', 'Binary', false],
                    ['E', 'Hexadecimal', false],
                ],
            ...
            [
                'kategori_id'       => $ipa,
                'tipe'              => Question::TIPE_BS,
                'teks_soal'         => '<p>[PUBLIK] Energi tidak dapat diciptakan atau dimusnahkan, hanya dapat berubah bentuk. Pernyataan ini dikenal sebagai Hukum Kekekalan Energi.</p>',
                'tingkat_kesulitan' => 'mudah',
                'bobot'             => 1,
                'visibilitas'       => Question::VISIBILITAS_PUBLIK,
                'created_by'        => $this->guruId,
        ...
        foreach ($publiks as $data) {
            ...
            if (isset($data['kunci_bs'])) {
                QuestionOption::create(['question_id' => $q->id, 'kode_opsi' => 'B', 'teks_opsi' => 'Benar', 'is_correct' => $data['kunci_bs'],  'bobot_persen' => 100, 'urutan' => 0, 'aktif' => true]);
                QuestionOption::create(['question_id' => $q->id, 'kode_opsi' => 'S', 'teks_opsi' => 'Salah', 'is_correct' => !$data['kunci_bs'], 'bobot_persen' => 100, 'urutan' => 1, 'aktif' => true]);
            }
        }

        // ── Soal dengan Audio (demo URL) ───────────────────────────────────
        $audioSoal = [
            [
                'kategori_id'       => $bind,
                'tipe'              => Question::TIPE_PG,
                'teks_soal'         => '<p>[AUDIO DEMO] Dengarkan rekaman percakapan berikut, kemudian tentukan topik utama yang dibicarakan oleh kedua pembicara!</p>',
                'tingkat_kesulitan' => 'sedang',
                'bobot'             => 2,
                'visibilitas'       => Question::VISIBILITAS_PRIVATE,
                'audio_url'         => 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3',
                'audio_play_limit'  => 3,
                'audio_auto_play'   => false,
                'created_by'        => $this->guruId,
                'options'           => [
                    ['A', 'Rencana liburan bersama', false],
                    ['B', 'Jadwal ujian semester', true],
                    ['C', 'Kegiatan ekstrakurikuler', false],
                    ['D', 'Pelajaran Bahasa Indonesia', false],
                    ['E', 'Tugas kelompok', false],
                ],
            ],
            [
                'kategori_id'       => $ipa,
                'tipe'              => Question::TIPE_ISIAN,
                'teks_soal'         => '<p>[AUDIO DEMO] Dengarkan penjelasan eksperimen fisika berikut. Berapa nilai percepatan gravitasi bumi yang digunakan dalam percobaan tersebut (dalam m/s²)?</p>',
                'tingkat_kesulitan' => 'sedang',
                'bobot'             => 1,
                'visibilitas'       => Question::VISIBILITAS_INTERNAL,
        ...
        foreach ($audioSoal as $data) {
            ...
            $q = Question::create([
                ...
                'audio_play_limit'  => $data['audio_play_limit'],
                'audio_auto_play'   => $data['audio_auto_play'],
                'aktif'             => true,
                'created_by'        => $data['created_by'],
            ]);
            foreach ($data['options'] ?? [] as $i => [$kode, $teks, $benar]) {
        ...
        $this->command->line('  <info>✓</info> Demo visibilitas (private/internal/publik) dan audio berhasil dibuat.');
    }
    ...
    private function makeBS(int $kategoriId, string $teks, bool $kunciBenar, string $kesulitan = 'mudah', float $bobot = 1, ?string $penjelasan = null): void
    {
        if (Question::where('teks_soal', $teks)->exists()) {
            return;
        }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // HELPER: makeCloze
    // blanks = [['urutan', 'placeholder', 'jawaban_benar', 'keywords_json', 'case_sensitive'], ...]
    // ─────────────────────────────────────────────────────────────────────────

    private function makeCloze(int $kategoriId, string $teks, array $blanks, string $kesulitan = 'sedang', float $bobot = 2, ?string $penjelasan = null): void
    {
        if (Question::where('teks_soal', $teks)->exists()) {
            return;
        }
        ...
        foreach ($blanks as $blank) {
            QuestionClozeBlank::create([
                'question_id'   => $q->id,
                'urutan'        => $blank['urutan'],
                'placeholder'   => $blank['placeholder'],
                'jawaban_benar' => $blank['jawaban_benar'],
                'keywords_json' => $blank['keywords_json'],
