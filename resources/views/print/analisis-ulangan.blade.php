...
<head>
    ...
    <title>Analisis Ulangan — {{ $session->nama_sesi }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    @php
        $teacherNip = $session->creator?->nip ?: $session->creator?->nomor_peserta ?: '—';
    @endphp

    {{-- ══════════════════════════════════════════════════════════════════════════ --}}
    {{-- HALAMAN COVER — SAMPUL ANALISIS ULANGAN --}}
    {{-- ══════════════════════════════════════════════════════════════════════════ --}}
        ...
        <div style="text-align: left; margin-top: auto; font-size: 11pt; line-height: 1.8;">
            <table style="width: 100%; border: none;">
                ...
                <tr>
                    ...
                    <td style="border: none; padding: 3px 0;">: {{ $teacherNip }}</td>
                </tr>
                <tr>
    ...
    {{-- ══════════════════════════════════════════════════════════════════════════ --}}
    {{-- HALAMAN 1 — ANALISIS ULANGAN --}}
    {{-- ══════════════════════════════════════════════════════════════════════════ --}}
    <div class="halaman">

        <div class="judul">Analisis Ulangan</div>
        @if ($schoolName)
        ...
        <table class="info">
            <tr>
                ...
                <td class="sep">:</td>
                <td>{{ $analisisData['session']->package?->nama ?? '—' }}</td>
            ...
            <tr>
                ...
                <td class="lbl">Jumlah Soal</td>
                <td class="sep">:</td>
                <td>{{ $analisisData['soal_list']->count() }}</td>
            </tr>
            <tr>
                <td class="lbl">Tanggal</td>
                <td class="sep">:</td>
                <td>{{ $session->waktu_mulai?->format('d F Y') ?? '—' }}</td>
                <td class="lbl">Bentuk Soal</td>
                <td class="sep">:</td>
                <td>Essay / Pilihan Ganda</td>
            </tr>
            <tr>
                <td class="lbl">KKM</td>
                <td class="sep">:</td>
                <td>{{ $analisisData['session']->kkm ?? 70 }}</td>
        ...
        <table class="data">
            ...
            <tfoot>
                ...
                <tr class="total-row">
                    <td colspan="2" style="text-align:left;">Skor {{ $analisisData['session']->kkm ?? 70 }} –
                        {{ $analisisData['session']->pengayaan_max_1 ?? 83 }}
                    </td>
                    @foreach ($analisisData['soal_stats'] as $stat)
                        <td></td>
                    @endforeach
                ...
                <tr class="total-row">
                    <td colspan="2" style="text-align:left;">Skor
                        {{ ($analisisData['session']->pengayaan_max_1 ?? 83) + 1 }} –
                        {{ $analisisData['session']->pengayaan_max_2 ?? 92 }}
                    </td>
                    @foreach ($analisisData['soal_stats'] as $stat)
                        <td></td>
                    @endforeach
                ...
                <tr class="total-row">
                    <td colspan="2" style="text-align:left;">Skor
                        {{ ($analisisData['session']->pengayaan_max_2 ?? 92) + 1 }} – 100
                    </td>
                    @foreach ($analisisData['soal_stats'] as $stat)
                        <td></td>
                    @endforeach
        ...
        <div class="ttd-area">
            ...
            <div class="ttd-block">
                ...
                <div class="nip">NIP. {{ $teacherNip }}</div>
            </div>
        </div>
    </div>
    ...
    {{-- ══════════════════════════════════════════════════════════════════════════ --}}
    {{-- HALAMAN 3 — HASIL ANALISIS ULANGAN --}}
    {{-- ══════════════════════════════════════════════════════════════════════════ --}}
    <div class="halaman halaman-portrait">

        <div class="judul">Hasil Analisis Ulangan</div>
        @if ($schoolName)
            <div class="sekolah">{{ strtoupper($schoolName) }}</div>
        @endif
        ...
        <div class="has-section">
            ...
            <div style="margin-left:16px;">
                ...
                <ul class="has-list">
                    ...
                    <li>Jumlah peserta yang telah tuntas belajar: <strong>{{ $hasilData['total_tuntas'] }}</strong>
                        siswa</li>
                    <li>Jumlah peserta yang belum tuntas belajar:
                        <strong>{{ $hasilData['total_tidak_tuntas'] }}</strong> siswa
                    </li>
                </ul>
            </div>
            <div style="margin-left:16px;margin-top:8px;">
                ...
                <ul class="has-list">
                    ...
                    <li>Jumlah soal yang telah tuntas klasikal: <strong>{{ $hasilData['soal_tuntas_count'] }}</strong>
                        butir</li>
                    <li>Jumlah soal yang belum tuntas klasikal:
                        <strong>{{ $hasilData['soal_tidak_tuntas_count'] }}</strong> butir
                    </li>
                </ul>
            </div>
        </div>
    ...
    <div class="halaman halaman-portrait">
        ...
        <table class="info">
            ...
            <tr>
                <td class="lbl">Jumlah Peserta Tuntas</td>
                <td class="sep">:</td>
